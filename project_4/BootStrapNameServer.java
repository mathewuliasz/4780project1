...
public class BootStrapNameServer {
    ...
    public static void main(String[] args) throws Exception {
        if (args.length < 1) {
            System.out.println("Usage: java BootStrapNameServer <config-file>");
            return;
        }
        Path configFile = Paths.get(args[0]);
        if (!Files.exists(configFile)) {
            System.out.println("The required bootstrap descriptor file cannot be found.");
            return;
        }
        List<String> lines = Files.readAllLines(configFile);
        int port = Integer.parseInt(lines.get(1).trim());
        String ip = InetAddress.getLocalHost().getHostAddress();
        BootStrapNameServer server = new BootStrapNameServer(ip, port);

        // load initial key-value pairs starting at line 2
        for (int i = 2; i < lines.size(); i++) {
            String line = lines.get(i).trim();
            if (line.isEmpty())
                continue;
            String[] parts = line.split(" ");
            server.keys.put(Integer.parseInt(parts[0]), parts[1]);
        }
        server.start();
    }
}
